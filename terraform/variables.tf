...
variable "scan_candidates_per_cron_tick" {
  description = "Candidates per `ichnos scan` invocation - sized so one run takes roughly as long as the cron interval between invocations (see cli.py's module docstring). Native ZMap discovery runs at config.py's zmap_rate_pps (16 pps), so one run takes roughly `candidates / rate_pps + cooldown_seconds` - at the defaults, 52000 candidates takes ~3253s (~54.2 min), leaving a 347s buffer inside the hourly cron interval. Sized on *relative* buffer, not absolute: run-to-run variance comes from the serial ZGrab2 backlog, which scales with the candidate count, so the buffer has to scale with it too. 21 measured runs at 12800-in-15-minutes had a median of 803.3s against a predicted 803.0 but a worst case of 864.3s (+7.64%, an https run - highest hit rate, most grabs, most 30s timeouts), comfortably inside that window's 12.1% buffer. Reproducing that same relative overrun on an hourly run needs ~10% of headroom to survive it: 56000 leaves 2.77% and would overrun by 170s, 54000 leaves 6.57% and still overruns. Because the cron entries are flock-guarded (user_data.sh.tftpl) an overrun does not just run long - it skips the following hour, so the cost of undersizing the buffer is a lost hour, not a late finish. Note this is not a throughput change: 1,248,000 per protocol per day against 1,228,800 on 15-minute slices, ~1.6%. Nearly all of the ~9% an unsafe 56000 appeared to offer was margin, not amortisation; the real fixed-overhead saving (72 cooldowns a day rather than 288) is 216s, or 0.25%. The trade being made is coarser observability and a larger unit of loss on an unclean exit, since cmd_scan writes nothing to pending_dir until a run completes."
  type        = number
  default     = 52000
}
