...
variable "scan_candidates_per_cron_tick" {
  description = "Candidates per `ichnos scan` invocation - sized so one run takes roughly as long as the cron interval between invocations (see cli.py's module docstring). At the default 5s/request budget shared across http+https and a 15-minute cron interval, ~40 is a reasonable starting point per protocol."
  type        = number
  default     = 40
}
